...
if not firebase_admin._apps:
    try:
        # Option 1: Environment variable (for Render/production - recommended)
        firebase_json = os.getenv("FIREBASE_SERVICE_ACCOUNT")
        if firebase_json:
            cred = credentials.Certificate(json.loads(firebase_json))
        else:
            # Option 2: Local JSON file (for development only)
...
def support():
    if not db:
        return jsonify({"reply": "Database service is not available."}), 503
    try:
        ...
        # Determine user_id (prefer explicit id, fallback to name)
        user_id = user.get("id") or user.get("name") or "anonymous"
        # Log to Firebase
        log_data = {
            ...
            "question": message,
            "answer": reply,
            "category": intent, # Used for ring chart
            "agent_id": agent_id
        }
        db.collection("agents").document("echo-support").collection("logs").add(log_data)
...
# ====================== Dashboard APIs ======================
# Get chat logs for a specific agent (latest 100)
...
# Get metrics for ring chart (category percentages)
@app.route("/api/metrics/<agent_id>", methods=["GET"])
def get_metrics(agent_id):
    if not db:
        return jsonify({"error": "Database unavailable"}), 503
    try:
        logs_ref = db.collection("agents").document(agent_id).collection("logs").stream()
        category_count = {}
        total = 0
        for doc in logs_ref:
            data = doc.to_dict()
